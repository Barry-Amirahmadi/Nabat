 ...
 */
const raw = process.env.SMOKE_BASE_PATH ?? "/nabat";
const BASE_PATH = raw === "/" ? "" : raw.replace(/\/+$/, "");
/**
 * This project's own port. Several sibling template repos ship the same
 * `serve-static.mjs`, and with `reuseExistingServer` a leftover server from
 * one of them answers here under a different base path — every route 404s and
 * the suite fails for a reason that is nowhere in this repository.
 */
const PORT = Number(process.env.SMOKE_PORT ?? 4356);
