...
## 54 — NABAT: THE ENGINE IS KEPT, THE DESIGN IS REPLACED, AND THE DENSITY COMES FROM GEOMETRY

This repository is `nabat-confectionery`, forked from `parnian-cosmetics` at the
point that site went live. **Everything above §54 still describes this codebase**
— Next 16 static export, `output: 'export'`, `trailingSlash`, the base-path
handling of §41 and §53, the typed content layer of §52, the smoke suite, the
three-runtime-dependency rule. None of it was touched. What was replaced is the
design: palette, typefaces, layout system, components, copy, every route's
composition. Read the sections above for the engine; read this one for what is
different.

**The pattern system is the new load-bearing idea, and it is why this site ships
with eight image files instead of fifty.** `src/components/pattern/Pattern.tsx`
draws four tileable geometries — an eight-pointed star tile, a carpet border,
sugar dots, repeating arches — as SVG `<pattern>` paint servers, each taking a
hue token and one of three cell sizes. Four geometries × six hues × three scales
is seventy-two grounds from about 4KB of component. A maximalist page built from
photographs needs a photograph per surface; one built from geometry needs a
colour token. Measured on the built output: twelve pattern layers on the
homepage, 9.8KB of inline pattern markup, 92KB of HTML.

Three rules hold it together and each has a failure mode behind it:

1. **`id` is required and unique per document.** A `<pattern>` is referenced by
   fragment id; two instances sharing one do not error, the second silently
   paints with the first one's geometry.
2. **Colour arrives as a CSS custom property through `style`, never as an SVG
   `stroke` attribute** — `stroke="var(--color-tala)"` as XML does not resolve,
   the CSS property does. That keeps `tokens.css` the only place a colour is
   spelled and lets the verification pass read the computed stroke back out of
   the DOM rather than being told what it should be.
3. **Pattern never sits behind body text.** Body text sits on an opaque plate;
   the plate sits on the pattern. Display type over pattern is fine and is
   measured against the composited stroke colour, not against the ground.

Rule 3 is the one that actually failed in build and had to be fixed in five
places — breadcrumbs, a transliteration, a derived count, the outlined detail
chips, and the group chip. `.plate-own` and `.on-band` in `globals.css` are the
two primitives that answer it: both paint an opaque patch of the band's *own*
colour, so they are invisible as panels and visible only as the absence of
pattern.

**Two things worth knowing before changing any grid here.** First, every cell on
this site states its row *and* column explicitly at every breakpoint, and the
placement tables carry `data-grid-*` attributes that `scripts/verify.mjs` reads
back. That is not belt-and-braces: sparse auto-placement never moves its cursor
backwards, so a cell asking for an earlier column is pushed into a new implicit
row — silently, with a green build and a clean console. Second, static
composition angles are set with the `rotate` property while the reveal animates
`transform`, which is what lets `prefers-reduced-motion` clear the motion
without flattening the design.

**`scripts/verify.mjs` is new and is the deliverable this repo would be weakest
without.** One script, every route, three widths, numbers only — statuses,
console errors, `scrollWidth` against `clientWidth`, contrast of every text node
against its *actual* composited background, computed grid placement against the
declared table, the rotated bounding box of every rotated element against the
viewport, focus-ring contrast on every ground the page paints, the decorative
layers' `aria-hidden` and focusability, per-route HTML and image weight, reduced
motion, and per-character glyph coverage of all three faces. It found nine
distinct contrast defects that no screenshot would have shown, including a chip
at 1.79:1. Nothing in this repository was ever judged by looking at it.
